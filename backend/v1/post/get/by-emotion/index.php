...
if ($c = new mysqli($localhost_db, $username_db, $password_db, $name_db)) {
    ...
    // mandatory emotion-id
    if (!isset($post["emotion-id"]) || !checkFieldValidity($post["emotion-id"])) {
        responseError(400, "Missing or wrong parameters: emotion-id");
    }
    $emotion_id = $post["emotion-id"];

    // optional only-from-users-followed
    $only_from_users_followed = false;
    if (isset($post['only-from-users-followed'])) {
        $only_from_users_followed = filter_var($post['only-from-users-followed'], FILTER_VALIDATE_BOOLEAN);
    }
    ...
    if ($user_id === null && isset($post['language']) && preg_match('/^[a-z]{2}$/i', $post['language'])) {
