...
// username is optional: accept either username (GET) or login-id (POST)
$has_username = isset($get["username"]) && checkUsernameValidity($get["username"]);
$has_login_id = isset($post["login-id"]) && checkFieldValidity($post["login-id"]);

if (!$has_username && !$has_login_id) {
    // bad request: missing both username and login-id
    $missing_parameters = array();
    if (!$has_username) array_push($missing_parameters, "username");
    if (!$has_login_id) array_push($missing_parameters, "login-id");
...
if ($c = new mysqli($localhost_db, $username_db, $password_db, $name_db)) {
    ...
    // If login-id provided, try to resolve to user-id (only active users, valid login)
    if ($login_id !== null) {
        ...
        try {
            ...
            if ($result->num_rows === 1) {
                ...
                // login-id not found or expired -> treat as anonymous (do not abort here)
                $login_in_user_id = null;
            }
    ...
    // If username provided: get user by username (only active users)
    if ($has_username) {
        ...
        // No username provided -> must have a valid login resolved to a user
        if ($login_in_user_id === null) {
            responseError(400, "Missing or wrong parameters: username or valid login-id required.");
        }

        // Get user info by user-id (active user)
        $query_get_by_userid = "SELECT `user-id`, `username`, `profile-image`, `bio` FROM $users_table WHERE `user-id` = ? AND `status` = 1";
        $stmt_get_by_userid = $c->prepare($query_get_by_userid);
        $stmt_get_by_userid->bind_param("s", $login_in_user_id);

        try {
            $stmt_get_by_userid->execute();
            $result_user = $stmt_get_by_userid->get_result();
            ...
            $row_user = $result_user->fetch_assoc();
            $username_user_id = $row_user["user-id"];
            $username = isset($row_user["username"]) ? $row_user["username"] : null;
            $profile_image = isset($row_user["profile-image"]) ? $row_user["profile-image"] : null;
            $bio = isset($row_user["bio"]) ? $row_user["bio"] : null;
        } catch (mysqli_sql_exception $e) {
            responseError(500, "Database error: " . $e->getMessage());
        }
        $stmt_get_by_userid->close();
    }
    ...
    if ($login_in_user_id !== null && strval($login_in_user_id) === strval($username_user_id)) {
        ...
        $data = array(
            ...
            "users-followed-count" => $following_count,
            "is-following" => null,
            "is-own-profile" => true
        );
        ...
        $data = array(
            ...
            "emotions-followed-count" => null,
            "users-followed-count" => null,
            "is-following" => $is_following,
            "is-own-profile" => $is_own_profile
        );
