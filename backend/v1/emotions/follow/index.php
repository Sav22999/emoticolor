...
if ($condition) {
    ...
    if ($c = new mysqli($localhost_db, $username_db, $password_db, $name_db)) {
        ...
        try {
            $stc->execute();
            $rf = $stc->get_result();
            if ($rf->num_rows === 1) responseError(409, "Emotion already followed.");
        } catch (mysqli_sql_exception $e) {
            responseError(500, "Database error: " . $e->getMessage());
        }
        $stc->close();
        ...
        // 4) insert follow
        $q_ins = "INSERT INTO $emotions_followed_table (`follow-id`, `user-id`, `created`, `emotion-id`) VALUES (NULL, ?, CURRENT_TIMESTAMP, ?)";
        $st_ins = $c->prepare($q_ins);
        if ($st_ins === false) responseError(500, "Database prepare error: " . $c->error);
        $st_ins->bind_param("ss", $user_id, $emotion_id);
        try {
            $st_ins->execute();
