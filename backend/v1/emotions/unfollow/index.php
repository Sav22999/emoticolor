    ...
    if ($c = new mysqli($localhost_db, $username_db, $password_db, $name_db)) {
        ...
        try {
            ...
            if ($result->num_rows === 1) {
                    ...
                    if ($result_check_emotion->num_rows === 1) {
                        ...
                        try {
                            ...
                            if ($result_get_user_id->num_rows === 1) {
                                ...
                                //remove followed emotion
                                $query_remove_follow = "DELETE FROM $emotions_followed_table WHERE `user-id` = ? AND `emotion-id` = ?";
                                $stmt_remove_follow = $c->prepare($query_remove_follow);
                                $stmt_remove_follow->bind_param("ss", $user_id, $emotion_id);
                ...
                responseError(404, "Emotion not followed.");
            }
        } catch (mysqli_sql_exception $e) {
            responseError(500, "Database error: " . $e->getMessage());
        }
