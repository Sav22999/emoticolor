...
$condition = isset($post["login-id"]) && checkFieldValidity($post["login-id"]) && isset($post["bio"]);
if ($condition) {
    $response = null;
    ...
    if ($c = new mysqli($localhost_db, $username_db, $password_db, $name_db)) {
        ...
        try {
            ...
            if ($result->num_rows === 1) {
                ...
                $bio = $post["bio"];
                $query_update_bio = "UPDATE $users_table SET `bio` = ? WHERE `user-id` = ? LIMIT 1";
                $stmt_update_bio = $c->prepare($query_update_bio);
                $stmt_update_bio->bind_param("ss", $bio, $user_id);
                try {
