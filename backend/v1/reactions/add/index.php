    ...
    if ($c = new mysqli($localhost_db, $username_db, $password_db, $name_db)) {
        ...
        try {
            ...
            if ($result->num_rows === 0) {
                            ...
                            if ($result_get_user_id->num_rows === 1) {
                                ...
                                try {
                                    ...
                                    if ($result_check_post->num_rows === 1) {
                                        ...
                                        $query_add_reaction = "INSERT INTO $reactions_posts_table (`post-id`, `user-id`, `reaction-id`, `created`) VALUES (?, ?, ?, CURRENT_TIMESTAMP)";
                                        $stmt_add_reaction = $c->prepare($query_add_reaction);
                                        $stmt_add_reaction->bind_param("ssi", $post_id, $user_id, $reaction_id);

                                        try {
                                            $stmt_add_reaction->execute();

                                            responseSuccess(204, null, null);
                                        } catch (mysqli_sql_exception $e) {
                                            responseError(500, "Database error: " . $e->getMessage());
                                        }
                                        $stmt_add_reaction->close();
                                    } else {
                ...
                responseError(404, "Reaction already added.");
            }
        } catch (mysqli_sql_exception $e) {
            responseError(500, "Database error: " . $e->getMessage());
        }
